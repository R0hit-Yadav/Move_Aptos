...
{ 
    ...
    const E_STARTING_PRICE_IS_LESS: u64 = 0;
    const E_NOT_ENOUGH_COINS: u64 = 1;
    const E_PLAYERS_LESS_THAN_THREE: u64 = 2;
    const E_INVALID_REWARD_AMOUNT: u64 = 3;
    const E_LESS_PRICE: u64 = 4;
    ...
    public fun pickWinner(acc: &signer) acquires Lotts
    {
        ...
        assert_is_owner(addr);
        assert!(total_players>=3, E_PLAYERS_LESS_THAN_THREE);
    ...
    #[test(admin = @ro_add,aptos_framework = @aptos_framework)]
    fun test_flow(admin: signer , aptos_framework: &signer) acquires Lotts 
    {
        ...
        let bet5 = account::create_account_for_test(@0x7);
        ...
        print(&utf8(b"Total Players"));
        print(&totalPlayers);


        let balance = getBalance(&admin);
        print(&utf8(b"Total Balance"));
        print(&balance);
