...
Locked coins flow:
 ...
 2. Sponsor accounts (sponsors) call initialize_sponsor with the appropriate CoinType to set up their account for creating locks later.
 3. Sponsors add locked APTs for custom expiration time + amount for recipients. Each lockup is called a "lock".
 4. Sponsors can revoke a lock or change lockup (reduce or extend) anytime. This gives flexibility in case of contract violation or special 
    circumstances. If this is not desired, the deployer can remove these functionalities before deploying. 
    If a lock is canceled, the locked coins will be sent back to the withdrawal address. 
    This withdrawal address is set when initilizing the sponsor account and can only be changed when there are no active or unclaimed locks.
 5. Once the lockup has expired, the recipient can call claim to get the unlocked tokens.
 */

module rohit_add::locked_coins {
    ...
    public entry fun update_withdrawal_address<CoinType>(sponsor: &signer, new_withdrawal_address: address) acquires Locks 
    {
    ...
    public entry fun batch_add_locked_coins<CoinType>(sponsor: &signer, recipients: vector<address>, amounts: vector<u64>, unlock_time_secs: u64) acquires Locks 
    {
    ...
    public entry fun add_locked_coins<CoinType>(sponsor: &signer, recipient: address, amount: u64, unlock_time_secs: u64) acquires Locks 
    {
        let sponsor_address = signer::address_of(sponsor);
        assert!(exists<Locks<CoinType>>(sponsor_address), error::not_found(ESPONSOR_ACCOUNT_NOT_INITIALIZED));
    ...
    public entry fun batch_update_lockup<CoinType>(sponsor: &signer, recipients: vector<address>, new_unlock_time_secs: u64) acquires Locks 
    {
        let sponsor_address = signer::address_of(sponsor);
        assert!(exists<Locks<CoinType>>(sponsor_address), error::not_found(ESPONSOR_ACCOUNT_NOT_INITIALIZED));
    ...
    public entry fun update_lockup<CoinType>(sponsor: &signer, recipient: address, new_unlock_time_secs: u64) acquires Locks 
    {
        let sponsor_address = signer::address_of(sponsor);
        assert!(exists<Locks<CoinType>>(sponsor_address), error::not_found(ESPONSOR_ACCOUNT_NOT_INITIALIZED));
        let locks = borrow_global_mut<Locks<CoinType>>(sponsor_address);
    ...
    public entry fun test_recipient_can_claim_coins(aptos_framework: &signer, sponsor: &signer, recipient: &signer) acquires Locks 
    {
    ...
    public entry fun test_recipient_cannot_claim_coins_if_lockup_has_not_expired(aptos_framework: &signer, sponsor: &signer, recipient: &signer) acquires Locks 
    {
        let burn_cap = setup(aptos_framework, sponsor);
        let recipient_addr = signer::address_of(recipient);
    ...
    public entry fun test_recipient_cannot_claim_twice(aptos_framework: &signer, sponsor: &signer, recipient: &signer) acquires Locks 
    {
        let burn_cap = setup(aptos_framework, sponsor);
        let recipient_addr = signer::address_of(recipient);
    ...
    #[test(aptos_framework = @0x1, sponsor = @0x123, recipient = @0x234)]
    public entry fun test_sponsor_can_update_lockup(aptos_framework: &signer, sponsor: &signer, recipient: &signer) acquires Locks 
    {
        let burn_cap = setup(aptos_framework, sponsor);
        let recipient_addr = signer::address_of(recipient);
    ...
    public entry fun test_sponsor_can_batch_update_lockup(aptos_framework: &signer, sponsor: &signer, recipient_1: &signer, recipient_2: &signer) acquires Locks 
    {
        let burn_cap = setup(aptos_framework, sponsor);
    ...
    public entry fun test_sponsor_can_cancel_lockup(aptos_framework: &signer, sponsor: &signer, recipient: &signer, withdrawal: &signer) acquires Locks 
    {
        let burn_cap = setup(aptos_framework, sponsor);
        let recipient_addr = signer::address_of(recipient);
    ...
    public entry fun test_sponsor_can_batch_cancel_lockup(aptos_framework: &signer,sponsor: &signer,recipient_1: &signer,recipient_2: &signer,withdrawal: &signer,) acquires Locks 
    {
        let burn_cap = setup(aptos_framework, sponsor);
        let recipient_1_addr = signer::address_of(recipient_1);
        let recipient_2_addr = signer::address_of(recipient_2);
    ...
    public entry fun test_cannot_change_withdrawal_address_if_active_locks_exist(aptos_framework: &signer,sponsor: &signer,recipient: &signer,withdrawal: &signer,) acquires Locks 
    {
        let burn_cap = setup(aptos_framework, sponsor);
        let recipient_addr = signer::address_of(recipient);
        let withdrawal_addr = signer::address_of(withdrawal);
    ...
    #[test(aptos_framework = @0x1, sponsor = @0x123, recipient = @0x234, withdrawal = @0x456)]
    public entry fun test_can_change_withdrawal_address_if_no_active_locks_exist(aptos_framework: &signer,sponsor: &signer,recipient: &signer,withdrawal: &signer,) acquires Locks 
    {
        let burn_cap = setup(aptos_framework, sponsor);
        let recipient_addr = signer::address_of(recipient);
        let withdrawal_addr = signer::address_of(withdrawal);
